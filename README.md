...
- **Pedidos**: nombre y número del cliente, anticipo opcional, y los productos del pedido. Si el producto ya está en stock, la línea queda "Vendida" al instante (descuenta stock). Si el producto está "en camino", la línea queda "Bajo pedido" ligada al mismo envío del producto; al marcar ese envío "Entregado", la línea pasa sola a "Listo para entregar" (si el envío llega a tu Sucursal — ahí tú la marcas "Entregado" a mano cuando se la das al cliente) o directo a "Entregado" (si el envío va directo al Domicilio del cliente). Un pedido foráneo ("Envío foráneo") pide la dirección completa del cliente, para cuando el proveedor lo manda directo a su domicilio en vez de pasar por tu sucursal.
- **Categorías**: catálogo editable de tipos de producto (Jersey, Balón, Chamarra, Playera, o los que agregues) con subcategorías opcionales. Cada categoría dice si usa tallas y si es "tipo jersey" (pide manga/versión y habilita personalizado/parches). Productos y precios de proveedor siempre referencian la categoría por id (no por nombre), así que renombrarla no desconecta nada de lo ya guardado.
- **Proveedores**: nombre, WhatsApp (con link directo al chat) y un precio por cada categoría que te vendan — para categorías tipo jersey, un precio por combinación de manga/versión. Cada proveedor también tiene su propio costo extra por personalizar una pieza (nombre/número) y por cada parche, que se suman al precio base de esa pieza al calcular el costo.
- **Ventas**: ganancia real de lo entregado — precio de venta menos el costo del proveedor (precio base + parches + personalización si la línea la pidió) — congelado en el momento en que cada línea se marca "Entregado", para que editar precios de proveedor después no reescriba ventas ya cerradas.
